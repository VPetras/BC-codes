...
void adc_handler(bc_adc_channel_t channel, bc_adc_event_t event, void *event_param)
{
    static uint32_t ls = 0;
    static uint32_t fps = 0;
    uint32_t s = (bc_tick_get() / 1000);
    ...
    uint16_t value;
    bc_adc_async_get_value(BC_ADC_CHANNEL_A4, &value);
    value >>= 4;

    // bc_log_debug("v: %d %d", value, fps++);
    bc_adc_async_measure(BC_ADC_CHANNEL_A4);

    int max = ((float)LED_STRIP_COUNT/4096.0) * (float)value;

    bc_log_debug("v: %d %d", value, max);

    for (int i = 0; i < max; i++)
    {
        bc_led_strip_set_pixel_rgbw(&led_strip.self, i, 0xff,0,0,0);
    }
    for (int i = max; i < LED_STRIP_COUNT; i++)
    {
        bc_led_strip_set_pixel_rgbw(&led_strip.self, i, 0,0,0,0);
    }
...
void application_init(void)
{
    ...
    bc_adc_set_event_handler(BC_ADC_CHANNEL_A4, adc_handler, NULL);
    bc_adc_async_measure(BC_ADC_CHANNEL_A4);
